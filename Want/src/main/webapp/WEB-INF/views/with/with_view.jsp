...
withTO to = (withTO) request.getAttribute("to");
...
<!-- CSS File -->
<link href="./resources/css/with_view.css?1232" rel="stylesheet">
...
<body>
	...
	<section id="info" class="container">
		<table class="table table-hover" id='board_list'>
			<thead>
				<tr>
					<td id="location" class="text-left d-none d-md-table-cell"><b>[<%=location%>]
					</b></td>
				</tr>
				<tr>
	...
	<section id="section-picture" class="container">
		<div class="picture">
			<img src="./upload/with/<%=picture%>" width="600px"></img>
		</div>
	</section>
	...
	<section class="container" style="margin-top: 10px;">
		<form id="commentForm" name="commentForm" method="post">
			...
			<div>
				<div class="cmt">
					<span><strong>댓글 <span id="count_reply"
							style="color: #5fcf80;"><%=reply%></span></strong></span>
				</div>
				<hr />
				<br />
		...
		<div id="btn_buttons">
			<span>
				<button class="btn btn--radius-2 btn-sm btn--silver" type="button"
					onclick="location.href='./with_list.do'">목록</button> <!-- 로그인이 되어있고 작성자가 같을 때만 수정과 삭제버튼이 보이게 한다. -->
				...
				<span>
					<c:choose>
						...
						<c:otherwise>
							<button type="button"
								class="btn btn--radius-2 btn--blue-2 btn-md"
								onclick="location.href='./with_write.do?cpage=<%=cpage%>'">신청하기</button>
						</c:otherwise>
					</c:choose>
				</span>
			</span>
				...
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn--blue-2"
							onclick="location.href='./with_delete_ok.do?no=<%=request.getParameter("no")%>'">삭제</button>
					</div>
				</div>
				...
				<div class="modal-content">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn--silver"
							data-bs-dismiss="modal">취소</button>
						<button type="button" class="btn btn--blue-2"
							onclick="location.href='./with_modify.do?no=<%=request.getParameter("no")%>'">수정</button>
					</div>
				</div>
...
// 모댓글 작성
	const WriteReply = function(bno){
		...
		content = content.trim();
			...
			$.ajax({
				url: 'with_write_reply.do',
				type: 'get',
				data: {
					bno : bno,
					content : content
				},
				success: function(wto){
					alert("댓글 작성 성공");
				
					$("#count_reply").text(wto.reply);
