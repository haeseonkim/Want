...
<%
	...
	for (AccomTO to : list) {
		...
		sbHtml.append(
		" 		<a href='./accom_view.do?no=" + to.getNo() + "&location=" + location + "' class='text-dark'>");
		sbHtml.append(
		" 		<img src='./upload/accom/" + to.getPicture() + "' alt='' class='img-fluid card-img-top'>");
		sbHtml.append(" 		<div class='p-4'>");
		sbHtml.append(" 			<h5><a href='./accom_view.do?no=" + to.getNo() + "&location=" + location
		+ "' class='text-dark'>" + to.getSubject() + "</a></h5>");
		...
		sbHtml.append("			</a> ");
		sbHtml.append(" 				&nbsp;&nbsp;<span class='small text-muted mb-0'>" + to.getWriter() + " | "
		+ to.getWdate() + "</span>");
		sbHtml.append(
		" 			<div class='d-flex align-items-center justify-content-between rounded-pill bg-light px-3 py-2 mt-4 boardItem'>");
		if (nick == null) {
			sbHtml.append(
			...
			if (to.getHno() == null) {
		sbHtml.append("        <a idx='" + to.getNo() + "' href='javascript:' class='heart-click heart_icon"
				+ to.getNo() + "'>");
		sbHtml.append(
...
<body>
	...
	<div class="container-fluid">
		<div class="px-lg-5">
							...
							<div class="col-4 banner-container">
								...
								<div>
									<c:choose>
										...
										<c:otherwise>
											<button type="button" class="btn btn-write"
												onclick="location.href='./accom_write.do?location=<%=location%>'">정보
												공유하기</button>
										</c:otherwise>
									</c:choose>
								</div>
			...
			<!-- 검색기능 끝 -->
			<br /> <input type="hidden" class="heart_func_if" id="origin" />
			<!--게시판-->
