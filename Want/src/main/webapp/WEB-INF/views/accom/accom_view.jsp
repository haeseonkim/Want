...
<%@page import="com.exam.model1.accomComment.AccomCommentTO"%>
...
<%
	...
	ArrayList<AccomCommentTO> lists = (ArrayList)request.getAttribute( "lists" );
	StringBuffer sbHtml = new StringBuffer();
	
	for( AccomCommentTO commentTo : lists ) {
...
const deleteOk = function() {
	...
	if (confirm("해당 게시글을 삭제하시겠습니까?") == true){    //확인
		document.accom_frm.action = "./accom_delete_ok.do";
		document.accom_frm.submit();
	}else{   //취소
	    return false;
	}
}
...
const modifyOk = function() {
	...
	document.accom_frm.action = "./accom_modify.do";
	document.accom_frm.submit();
...
const reply_deleteOk = function(clicked_id) {
	...
	if (confirm("해당 댓글을 삭제하시겠습니까?") == true){    //확인
		...
		document.forms[frm_id].action = "./accom_reply_deleteOk.do";
...
const reply_modifyOk = function(clicked_id) {
	...
	document.forms[frm_id].action = "./accom_reply_modifyOk.do";
	document.forms[frm_id].submit();
}
...
const reply_commentOk = function(clicked_id) {
	...
	document.forms[clicked_id].action = "./accom_rereplyOk.do";
...
<jsp:include page="../include/navbar.jsp">
	<jsp:param value="accom_view" name="thisPage" />
</jsp:include>
...
<div class="view1">
	...
	<div class="view2">
		<div class="view3">
			...
			<!-- 댓글 div  -->
			<div class="view4">
				<form action="./accom_view_comment_ok.do" method="post" name="cfrm">
					<input type="hidden" name="no" value="<%=no %>" />
					<input type="hidden" name="cpage" value="<%=cpage %>" />
					<table class="table table-bordered table_reply">
					<tr>
						<td width="94%">
							<strong>Comments | <%= reply %></strong> <input type="hidden" name="cwriter"  value="<%= nick%>" readonly/>&nbsp;&nbsp;
						</td>
			...
			<tr>
				<td class="coment_re" width="20%">
					<div class="btn_list">
						<input type="button" id="submit_list" value="목록" class="btn btn-success" onclick="location.href='./accom_list.do?cpage=<%=cpage %>&location=<%=location%>'" />
					</div>
					<div class="btn_dm">
						<c:choose>		
							<c:when test="${empty sessionScope.nick}">
							...
							<c:otherwise>
								<form name="accom_frm" method="post" >
									<input type="hidden" name="no" value="<%=no %>"/>
