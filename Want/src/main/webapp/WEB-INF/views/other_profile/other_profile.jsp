...
<body>
			...
			<div class="col-sm-9">
				<ul class="nav nav-tabs" id="myTab" role="tablist">
					<li class="nav-item" role="presentation">
						<button class="nav-link active" id="lantrip-tab" data-bs-toggle="tab"
							data-bs-target="#lantrip" type="button" role="tab"
							aria-controls="home" aria-selected="true" other_nick="${to.nick }">랜선여행</button>
					</li>
					<li class="nav-item" role="presentation">
						<button class="nav-link" id="picture-tab" data-bs-toggle="tab"
							data-bs-target="#picture" type="button" role="tab"
							aria-controls="profile" aria-selected="false" other_nick="${to.nick }">사진자랑</button>
					</li>
					<li class="nav-item" role="presentation">
						<button class="nav-link" id="shopping-tab" data-bs-toggle="tab"
							data-bs-target="#shopping" type="button" role="tab"
							aria-controls="shopping" aria-selected="false" other_nick="${to.nick }">쇼핑정보</button>
					</li>
					<li class="nav-item" role="presentation">
						<button class="nav-link" id="accom-tab" data-bs-toggle="tab"
							data-bs-target="#accom" type="button" role="tab"
							aria-controls="accom" aria-selected="false" other_nick="${to.nick }">숙소정보</button>
					</li>
				</ul>
	...
	<script>
		...
		const GetLantripList = function(currentPage, other_nick){
			console.log("inGetLantripList"+currentPage);
			
			// 무한 스크롤
			$.ajax({
				url:"ajax_lantrip_page.do",
				method:"GET",
				//검색기능이 있는 경우 condition과 keyword를 함께 넘겨줘야한다. 안그러면 검색결과만 나와야하는데 다른것들이 덧붙여져 나온다.
				data:"pageNum="+currentPage+"&other_nick="+other_nick,
		...
		$("#lantrip-tab").on('click', function() {
			currentTab = "lantrip";
			currentPage = 1;
			GetLantripList(currentPage, $(this).attr("other_nick"));
		...
		$("#picture-tab").on('click', function() {
			currentTab = "picture";
			currentPage = 1;
			GetPictureList(currentPage, $(this).attr("other_nick"));
		...
		$("#shopping-tab").on('click', function() {
			currentTab = "shopping";
			currentPage = 1;
			GetShoppingList(currentPage, $(this).attr("other_nick"));
		...
		$("#accom-tab").on('click', function() {
			currentTab = "accom";
			currentPage = 1;
			GetAccomList(currentPage, $(this).attr("other_nick"));
			//alert("accom-tab 클릭");
		})
	</script>
